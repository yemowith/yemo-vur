...
contract CelerWallet is ICelerWallet, Pausable {
    ...
    modifier onlyOperator(bytes32 _walletId) {
        require(
            msg.sender == wallets[_walletId].operator,
            "msg.sender is not operator"
        );
        _;
    }
    ...
    modifier onlyWalletOwner(bytes32 _walletId, address _addr) {
        require(
            _isWalletOwner(_walletId, _addr),
            "Given address is not wallet owner"
        );
        _;
    }
    ...
    ) public whenNotPaused onlyOperator(_walletId) {
    ...
    function proposeNewOperator(
        bytes32 _walletId,
        address _newOperator
    ) public onlyWalletOwner(_walletId, msg.sender) {
     ...
     */
    function getProposedNewOperator(
        bytes32 _walletId
    ...
    function getProposalVote(
        bytes32 _walletId,
        address _owner
    ) external view onlyWalletOwner(_walletId, _owner) returns (bool) {
