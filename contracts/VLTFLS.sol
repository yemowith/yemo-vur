...
abstract contract YF is FlashLoanSimpleReceiverBase {
    ...
    function _aterFl(uint256 _amount) internal virtual returns (bool) {
        return true;
    }
...
contract VLTFLS is YF {
    ...
    constructor(address _addressProvider, address _weth) YF(_addressProvider) {
        weth = _weth;
    }
    ...
    function _bfrStrt(address _token, uint256 amount) internal returns (bool) {
        ...
        if (_bba < _r) {
            IWETH2(weth).deposit{value: _r}();
        }

        return true;
    }

    function _aterFl(uint256 _amount) internal pure override returns (bool) {
        return true;
    }
