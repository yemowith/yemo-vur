...
const balanceOf = async (address: string, token: string): Promise<any> => {
  ...
  logg.info(
    `Balance is ${balanceAmount} for address ${address} for token ${token}`
  );
  return balance;
};
...
const allowance = async (
  ...
  logg.info(
    `Allowance for owner ${owner}, spender ${spender}, token ${token}: ${fromEther(
      allowance
    )}`
  );
  return allowance;
...
const approve = async (
  ...
  const tokenContract = await ethers.getContractAt(IERC20, token);
  const approvalResult = await tokenContract.approve(spender, amount);
  logg.info(
...
const transfer = async (
  ...
  const tokenContract = await ethers.getContractAt(IERC20, token);
  const transferResult = await tokenContract.transfer(to, amount);
  logg.info(
    `Transferred amount ${ethers.utils.formatEther(
      amount
    )} to ${to} for token ${token}`
  );
