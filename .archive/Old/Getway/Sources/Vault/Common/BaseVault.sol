...
import {Ownable, Initializable, AdminAble, Pausable, Lockable} from './abilities.sol'; // MngdC kontratının yolu doğru olmalıdır.

contract BaseVault is Ownable, Pausable, AdminAble, Lockable {
    ...
    // Deposit ETH
    function depositEther() public payable whenNotPaused whenNotLocked {
    ...
    // Withdraw ETH
    function withdrawEther(uint256 amount) public whenNotPaused whenNotLocked {
    ...
    function depositToken(
        address token,
        uint256 amount
    ) external whenNotPaused whenNotLocked {
