/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */
import type {
  BaseContract,
  BigNumber,
  BytesLike,
  CallOverrides,
  ContractTransaction,
  Overrides,
  PayableOverrides,
  PopulatedTransaction,
  Signer,
  utils,
} from "ethers";
import type { FunctionFragment, Result } from "@ethersproject/abi";
import type { Listener, Provider } from "@ethersproject/providers";
import type {
  TypedEventFilter,
  TypedEvent,
  TypedListener,
  OnEvent,
  PromiseOrValue,
} from "../../../common";

export interface YemoKitInterface extends utils.Interface {
  functions: {
    "addCBTCD(bytes32,bytes)": FunctionFragment;
    "adrsb()": FunctionFragment;
    "dplr()": FunctionFragment;
    "dply(bytes,string)": FunctionFragment;
    "dplyPY(address)": FunctionFragment;
    "dsspf()": FunctionFragment;
    "eemt()": FunctionFragment;
    "evt(string,string)": FunctionFragment;
    "exe(address,bytes)": FunctionFragment;
    "init()": FunctionFragment;
    "intd()": FunctionFragment;
    "kill()": FunctionFragment;
    "owner()": FunctionFragment;
    "pys(address)": FunctionFragment;
    "stg()": FunctionFragment;
  };

  getFunction(
    nameOrSignatureOrTopic:
      | "addCBTCD"
      | "adrsb"
      | "dplr"
      | "dply"
      | "dplyPY"
      | "dsspf"
      | "eemt"
      | "evt"
      | "exe"
      | "init"
      | "intd"
      | "kill"
      | "owner"
      | "pys"
      | "stg"
  ): FunctionFragment;

  encodeFunctionData(
    functionFragment: "addCBTCD",
    values: [PromiseOrValue<BytesLike>, PromiseOrValue<BytesLike>]
  ): string;
  encodeFunctionData(functionFragment: "adrsb", values?: undefined): string;
  encodeFunctionData(functionFragment: "dplr", values?: undefined): string;
  encodeFunctionData(
    functionFragment: "dply",
    values: [PromiseOrValue<BytesLike>, PromiseOrValue<string>]
  ): string;
  encodeFunctionData(
    functionFragment: "dplyPY",
    values: [PromiseOrValue<string>]
  ): string;
  encodeFunctionData(functionFragment: "dsspf", values?: undefined): string;
  encodeFunctionData(functionFragment: "eemt", values?: undefined): string;
  encodeFunctionData(
    functionFragment: "evt",
    values: [PromiseOrValue<string>, PromiseOrValue<string>]
  ): string;
  encodeFunctionData(
    functionFragment: "exe",
    values: [PromiseOrValue<string>, PromiseOrValue<BytesLike>]
  ): string;
  encodeFunctionData(functionFragment: "init", values?: undefined): string;
  encodeFunctionData(functionFragment: "intd", values?: undefined): string;
  encodeFunctionData(functionFragment: "kill", values?: undefined): string;
  encodeFunctionData(functionFragment: "owner", values?: undefined): string;
  encodeFunctionData(
    functionFragment: "pys",
    values: [PromiseOrValue<string>]
  ): string;
  encodeFunctionData(functionFragment: "stg", values?: undefined): string;

  decodeFunctionResult(functionFragment: "addCBTCD", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "adrsb", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "dplr", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "dply", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "dplyPY", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "dsspf", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "eemt", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "evt", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "exe", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "init", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "intd", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "kill", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "owner", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "pys", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "stg", data: BytesLike): Result;

  events: {};
}

export interface YemoKit extends BaseContract {
  connect(signerOrProvider: Signer | Provider | string): this;
  attach(addressOrName: string): this;
  deployed(): Promise<this>;

  interface: YemoKitInterface;

  queryFilter<TEvent extends TypedEvent>(
    event: TypedEventFilter<TEvent>,
    fromBlockOrBlockhash?: string | number | undefined,
    toBlock?: string | number | undefined
  ): Promise<Array<TEvent>>;

  listeners<TEvent extends TypedEvent>(
    eventFilter?: TypedEventFilter<TEvent>
  ): Array<TypedListener<TEvent>>;
  listeners(eventName?: string): Array<Listener>;
  removeAllListeners<TEvent extends TypedEvent>(
    eventFilter: TypedEventFilter<TEvent>
  ): this;
  removeAllListeners(eventName?: string): this;
  off: OnEvent<this>;
  on: OnEvent<this>;
  once: OnEvent<this>;
  removeListener: OnEvent<this>;

  functions: {
    addCBTCD(
      _key: PromiseOrValue<BytesLike>,
      _data: PromiseOrValue<BytesLike>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<ContractTransaction>;

    adrsb(overrides?: CallOverrides): Promise<[string]>;

    dplr(overrides?: CallOverrides): Promise<[string]>;

    dply(
      _bytecode: PromiseOrValue<BytesLike>,
      _salt: PromiseOrValue<string>,
      overrides?: PayableOverrides & { from?: PromiseOrValue<string> }
    ): Promise<ContractTransaction>;

    dplyPY(
      _owner: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<ContractTransaction>;

    dsspf(overrides?: CallOverrides): Promise<[string]>;

    eemt(overrides?: CallOverrides): Promise<[string]>;

    evt(
      sub: PromiseOrValue<string>,
      data: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<ContractTransaction>;

    exe(
      _target: PromiseOrValue<string>,
      _data: PromiseOrValue<BytesLike>,
      overrides?: PayableOverrides & { from?: PromiseOrValue<string> }
    ): Promise<ContractTransaction>;

    init(
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<ContractTransaction>;

    intd(overrides?: CallOverrides): Promise<[boolean]>;

    kill(
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<ContractTransaction>;

    owner(overrides?: CallOverrides): Promise<[string]>;

    pys(
      arg0: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<[string]>;

    stg(overrides?: CallOverrides): Promise<[string]>;
  };

  addCBTCD(
    _key: PromiseOrValue<BytesLike>,
    _data: PromiseOrValue<BytesLike>,
    overrides?: Overrides & { from?: PromiseOrValue<string> }
  ): Promise<ContractTransaction>;

  adrsb(overrides?: CallOverrides): Promise<string>;

  dplr(overrides?: CallOverrides): Promise<string>;

  dply(
    _bytecode: PromiseOrValue<BytesLike>,
    _salt: PromiseOrValue<string>,
    overrides?: PayableOverrides & { from?: PromiseOrValue<string> }
  ): Promise<ContractTransaction>;

  dplyPY(
    _owner: PromiseOrValue<string>,
    overrides?: Overrides & { from?: PromiseOrValue<string> }
  ): Promise<ContractTransaction>;

  dsspf(overrides?: CallOverrides): Promise<string>;

  eemt(overrides?: CallOverrides): Promise<string>;

  evt(
    sub: PromiseOrValue<string>,
    data: PromiseOrValue<string>,
    overrides?: Overrides & { from?: PromiseOrValue<string> }
  ): Promise<ContractTransaction>;

  exe(
    _target: PromiseOrValue<string>,
    _data: PromiseOrValue<BytesLike>,
    overrides?: PayableOverrides & { from?: PromiseOrValue<string> }
  ): Promise<ContractTransaction>;

  init(
    overrides?: Overrides & { from?: PromiseOrValue<string> }
  ): Promise<ContractTransaction>;

  intd(overrides?: CallOverrides): Promise<boolean>;

  kill(
    overrides?: Overrides & { from?: PromiseOrValue<string> }
  ): Promise<ContractTransaction>;

  owner(overrides?: CallOverrides): Promise<string>;

  pys(arg0: PromiseOrValue<string>, overrides?: CallOverrides): Promise<string>;

  stg(overrides?: CallOverrides): Promise<string>;

  callStatic: {
    addCBTCD(
      _key: PromiseOrValue<BytesLike>,
      _data: PromiseOrValue<BytesLike>,
      overrides?: CallOverrides
    ): Promise<boolean>;

    adrsb(overrides?: CallOverrides): Promise<string>;

    dplr(overrides?: CallOverrides): Promise<string>;

    dply(
      _bytecode: PromiseOrValue<BytesLike>,
      _salt: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<string>;

    dplyPY(
      _owner: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<string>;

    dsspf(overrides?: CallOverrides): Promise<string>;

    eemt(overrides?: CallOverrides): Promise<string>;

    evt(
      sub: PromiseOrValue<string>,
      data: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<boolean>;

    exe(
      _target: PromiseOrValue<string>,
      _data: PromiseOrValue<BytesLike>,
      overrides?: CallOverrides
    ): Promise<string>;

    init(overrides?: CallOverrides): Promise<boolean>;

    intd(overrides?: CallOverrides): Promise<boolean>;

    kill(overrides?: CallOverrides): Promise<void>;

    owner(overrides?: CallOverrides): Promise<string>;

    pys(
      arg0: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<string>;

    stg(overrides?: CallOverrides): Promise<string>;
  };

  filters: {};

  estimateGas: {
    addCBTCD(
      _key: PromiseOrValue<BytesLike>,
      _data: PromiseOrValue<BytesLike>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<BigNumber>;

    adrsb(overrides?: CallOverrides): Promise<BigNumber>;

    dplr(overrides?: CallOverrides): Promise<BigNumber>;

    dply(
      _bytecode: PromiseOrValue<BytesLike>,
      _salt: PromiseOrValue<string>,
      overrides?: PayableOverrides & { from?: PromiseOrValue<string> }
    ): Promise<BigNumber>;

    dplyPY(
      _owner: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<BigNumber>;

    dsspf(overrides?: CallOverrides): Promise<BigNumber>;

    eemt(overrides?: CallOverrides): Promise<BigNumber>;

    evt(
      sub: PromiseOrValue<string>,
      data: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<BigNumber>;

    exe(
      _target: PromiseOrValue<string>,
      _data: PromiseOrValue<BytesLike>,
      overrides?: PayableOverrides & { from?: PromiseOrValue<string> }
    ): Promise<BigNumber>;

    init(
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<BigNumber>;

    intd(overrides?: CallOverrides): Promise<BigNumber>;

    kill(
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<BigNumber>;

    owner(overrides?: CallOverrides): Promise<BigNumber>;

    pys(
      arg0: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<BigNumber>;

    stg(overrides?: CallOverrides): Promise<BigNumber>;
  };

  populateTransaction: {
    addCBTCD(
      _key: PromiseOrValue<BytesLike>,
      _data: PromiseOrValue<BytesLike>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<PopulatedTransaction>;

    adrsb(overrides?: CallOverrides): Promise<PopulatedTransaction>;

    dplr(overrides?: CallOverrides): Promise<PopulatedTransaction>;

    dply(
      _bytecode: PromiseOrValue<BytesLike>,
      _salt: PromiseOrValue<string>,
      overrides?: PayableOverrides & { from?: PromiseOrValue<string> }
    ): Promise<PopulatedTransaction>;

    dplyPY(
      _owner: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<PopulatedTransaction>;

    dsspf(overrides?: CallOverrides): Promise<PopulatedTransaction>;

    eemt(overrides?: CallOverrides): Promise<PopulatedTransaction>;

    evt(
      sub: PromiseOrValue<string>,
      data: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<PopulatedTransaction>;

    exe(
      _target: PromiseOrValue<string>,
      _data: PromiseOrValue<BytesLike>,
      overrides?: PayableOverrides & { from?: PromiseOrValue<string> }
    ): Promise<PopulatedTransaction>;

    init(
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<PopulatedTransaction>;

    intd(overrides?: CallOverrides): Promise<PopulatedTransaction>;

    kill(
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<PopulatedTransaction>;

    owner(overrides?: CallOverrides): Promise<PopulatedTransaction>;

    pys(
      arg0: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<PopulatedTransaction>;

    stg(overrides?: CallOverrides): Promise<PopulatedTransaction>;
  };
}
