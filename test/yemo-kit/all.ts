...
describe("Deploy kit", function () {
    ...
    it("stream events from", async function () {
        ...
        it("sleep for 3 minutes", async function () {
            this.timeout(180000) // Extend timeout to accommodate sleep duration
            console.log("Sleeping for 3 minutes...")
            await new Promise((resolve) => setTimeout(resolve, 180000)) // Sleep for 3 minutes
            console.log("Woke up after 3 minutes.")
        })
    })
})
